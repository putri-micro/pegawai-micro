        ...
        DataManager.fetchData(detail.replace(':id', id))
            .then(function (response) {
                if (response.success) {
                    ...
                    const dateValue = response.data.tanggal ? response.data.tanggal.split('T')[0] : '';
                    $('#edit_tanggal').val(dateValue);
                    $('#edit_jenis_libur').val(response.data.jenis_libur).trigger('change');
                    $('#edit_nama_libur').val(response.data.nama_libur);
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    const input = {
                        ...
                        jenis_libur: $('#edit_jenis_libur').val(),
                        nama_libur: $('#edit_nama_libur').val(),
                        keterangan: $('#edit_keterangan').val()
                    };
                    const update = '{{ route('admin.master.libur.update', [':id']) }}';
